...
router.get("/event/:eventid", isAuthenticated, async (req, res, next) => {
  ...
  try {
    const response = await Comment.find({ idEvent: eventid }).populate(
      "idUser"
    );
    res.status(200).json(response);
  } catch (error) {
...
// PATCH "/api/comment/:commentid" => modificar un comment, el content
...
// DELETE "/api/comment/:commentid" => eliminar un comment
